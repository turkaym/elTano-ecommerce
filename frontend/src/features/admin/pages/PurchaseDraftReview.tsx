...
interface Props {
  ...
  onAddLine: (line: { productName: string; quantity: string; unit: string; unitPrice: string }) => void
  onUpdateLine: (lineId: string, line: { productName: string; quantity: string; unit: string; unitPrice: string }) => void
    ...
    <section className="admin-card procurement-review" aria-labelledby="draft-review-title">
      ...
      <div className="procurement-table-wrap">
        <table className="procurement-table">
          ...
          <thead><tr><th>Fila</th><th>Producto</th><th>Cantidad</th><th>Costo</th><th>Estado</th><th>Catálogo</th><th>Acciones</th></tr></thead>
...
function DraftLineRow({ line, pending, onUpdate, onDelete, onMatch }: { line: PurchaseDraftLine; pending: boolean; onUpdate: Props['onUpdateLine']; onDelete: Props['onDeleteLine']; onMatch: Props['onMatchLine'] }) {
  ...
  if (editing) return (
    <tr className="procurement-edit-row">
      ...
      <td><label><span className="sr-only">Precio unitario de fila {line.rowNumber}</span><input type="number" min="0.01" step="0.01" value={unitPrice} onChange={(event) => setUnitPrice(event.target.value)} required /></label></td>
      <td colSpan={3}><div className="procurement-row-actions"><button className="btn btn-primary" type="button" disabled={pending || !unitPrice} onClick={() => { onUpdate(line.id, { productName: name, quantity, unit, unitPrice }); setEditing(false) }}>Guardar</button><button className="btn btn-secondary" type="button" onClick={() => setEditing(false)}>Cancelar</button></div></td>
    </tr>
  )
  ...
  return (
    <tr>
      ...
      <td data-label="Costo">{costEvidence(line)}</td>
...
function AddLineForm({ onAdd, disabled }: { onAdd: Props['onAddLine']; disabled: boolean }) {
  ...
  function submit(event: FormEvent) { event.preventDefault(); onAdd({ productName: name.trim(), quantity, unit, unitPrice }); setName(''); setUnitPrice(''); setOpen(false) }
  ...
  return <form className="procurement-add-form" onSubmit={submit}><label className="admin-field"><span>Producto nuevo</span><input value={name} onChange={(event) => setName(event.target.value)} required /></label><label className="admin-field"><span>Cantidad nueva</span><input type="number" step="any" min="0.001" value={quantity} onChange={(event) => setQuantity(event.target.value)} required /></label><label className="admin-field"><span>Unidad nueva</span><select value={unit} onChange={(event) => setUnit(event.target.value)}><option value="UNIDAD">Unidad</option><option value="KG">Kilogramo</option></select></label><label className="admin-field"><span>Precio unitario (ARS)</span><input type="number" min="0.01" step="0.01" value={unitPrice} onChange={(event) => setUnitPrice(event.target.value)} required /></label><button className="btn btn-primary" disabled={disabled}>Agregar</button><button type="button" className="btn btn-secondary" onClick={() => setOpen(false)}>Cancelar</button></form>
}
...
function ConfirmedDraftEvidence({ draft, confirmation, onDownloadSource }: { draft: PurchaseDraft; confirmation: PurchaseDraftConfirmation | null; onDownloadSource: () => void }) {
  return <section className="admin-card procurement-review" aria-labelledby="draft-review-title">
    ...
    <div className="procurement-table-wrap"><table className="procurement-table"><caption>Detalle inmutable de la importación</caption><thead><tr><th>Fila</th><th>Origen</th><th>Cantidad original</th><th>Costo</th><th>Destino y referencia</th><th>Conversión</th><th>Variación canónica</th></tr></thead><tbody>{draft.lines.map((line) => <tr key={line.id}><td>{line.rowNumber}</td><td><strong>{line.productName}</strong></td><td>{line.sourceQuantity} {humanUnit(line.unit ?? '')}</td><td>{costEvidence(line)}</td><td><TargetLink line={line} /></td><td>× {line.conversion}</td><td><strong>+{line.canonicalDelta}</strong></td></tr>)}</tbody></table></div>
  </section>
}
...
function costEvidence(line?: PurchaseDraftLine) {
  if (line?.unitPrice == null || line.lineTotal == null) return 'Costo no disponible'
  return `ARS ${line.unitPrice} por ${line.pricingUnit === 'KG' ? 'kg' : 'unidad'} · Total ARS ${line.lineTotal}`
}
